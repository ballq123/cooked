...
class RNNCell(object):
    ...
    def forward(self, x, h_prev_t):
        ...
        """
        print("\n\n\nW_ih:", self.W_ih.shape)
        print("\nx:", x.shape)
        print("\nself.b_ih:", self.b_ih.shape)
        print("\nself.W_hh:", self.W_hh.shape)
        print("\nh_prev_t:", h_prev_t.shape)
        print("\nself.b_hh:", self.b_hh.shape)
        # print("\nx:", x.shape)
        self.x = x
        affineIn = (x @ self.W_ih.T) + self.b_ih
        affinePrev = (h_prev_t @ self.W_hh.T) + self.b_hh
        h_t = self.activation.forward(affineIn + affinePrev)
    ...
    def backward(self, delta, h_t, h_prev_l, h_prev_t):
        ...
        # print("other:", (1 - (h_t**2)).shape)
        dz = delta * (1 - (h_t**2))

        # 1) Compute the averaged gradients of the weights and biases
        self.dW_ih += dz.T @ h_prev_l
        self.dW_hh += dz.T @ h_prev_t
        self.db_ih += np.sum(dz, axis=0)
        self.db_hh += np.sum(dz, axis=0)
