...
class RNNPhonemeClassifier(object):
    ...
    def forward(self, x, h_0=None):
        ...
        for seq in range(seq_len):
            input_t = x[:, seq, :]  # (batch_size, input_size)
            for layer in range(self.num_layers):  # Iterate over layers
                if (layer == 0):
                    h_prev_l = input_t
                else:
                    h_prev_l = hidden[layer - 1]
                hidden[layer] = self.rnn[layer].forward(h_prev_l, hidden[layer])
            self.hiddens.append(hidden.copy())     

        # Get the outputs from the last time step using the linear layer and return it
        # <--------------------------
        last = hidden[-1]
        logits = self.output_layer.forward(last)
        ...
        for s in reversed(range(seq_len)):
            for l in reversed(range(self.num_layers)):
                if (l == 0):
                    ...
                    h_prev_l = self.hiddens[s][l - 1]
                h_prev_t = self.hiddens[s][l]
                # dh[l] += self.rnn[l].backward(dh[l], h_prev_t, h_prev_l, self.hiddens[s + 1][l])
                dh[l] = self.rnn[l].backward(dh[l], h_prev_t, h_prev_l, self.hiddens[s + 1][l])
                if l > 0: 
                    dh[l - 1] += dh[l]
