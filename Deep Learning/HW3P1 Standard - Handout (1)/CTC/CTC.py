...
class CTC(object):
	...
	def extend_target_with_blank(self, target):
		...
		skip_connect = [0] * N
		for i in range(N):
			if 2 <= i < N - 1 and (extended_symbols[i] != extended_symbols[i - 2]) and (extended_symbols[i] != self.BLANK):
				skip_connect[i] = 1
				continue
			skip_connect[i] = 0
	...
	def get_backward_probs(self, logits, extended_symbols, skip_connect):
		...
		for t in reversed(range(T - 1)):
			beta_hat[t][S-1] = logits[t, extended_symbols[S-1]]*beta_hat[t + 1][S - 1]
			for s in reversed(range(S - 1)):
				beta_hat[t][s] = beta_hat[t + 1][s] + beta_hat[t + 1][s + 1]
				if s <= S - 3 and extended_symbols[s] != extended_symbols[s + 2]:  
					beta_hat[t][s] = beta_hat[t][s] + beta_hat[t + 1][s + 2]
				beta_hat[t][s] *= logits[t, extended_symbols[s]]
        
		for t in reversed(range(T)):
			for s in reversed(range(S)):
				beta[t][s] = beta_hat[t][s] / logits[t, extended_symbols[s]]
	...
	def get_posterior_probs(self, alpha, beta):
		...
		for t in range(T):
			...
			for i in range(S):
				...
				sumgamma[t] = sumgamma[t] + gamma[t][i]
		for t in range(T):
			for i in range(S):
...
class CTCLoss(object):
    ...
    def forward(self, logits, target, input_lengths, target_lengths):
        ...
        for batch_itr in range(B):
            btl = self.target_lengths[batch_itr]
            bti = self.input_lengths[batch_itr]
            truncTarg = self.target[batch_itr, :btl]
            truncLog = self.logits[:bti, batch_itr]
            extend, conn = self.ctc.extend_target_with_blank(truncTarg)
            fp = self.ctc.get_forward_probs(truncLog, extend, conn)
            bp = self.ctc.get_backward_probs(truncLog, extend, conn)
			
            g = self.ctc.get_posterior_probs(fp, bp)
            self.gammas.append(g)
            total_loss[batch_itr] = -1 * np.sum(g * np.log(truncLog[:, extend]))
    ...
    def backward(self):
        ...
        for batch_itr in range(B):
            ...
            truncTarg = self.target[batch_itr, :btl]
            truncLog = self.logits[:bti, batch_itr, :]
            extend, _ = self.ctc.extend_target_with_blank(truncTarg)
            for t in range(self.input_lengths[batch_itr]):
                for s in range(len(extend)):
                    dY[t, batch_itr, extend[s]] -= self.gammas[batch_itr][t, s] / truncLog[t, extend[s]]
